...
long random_prime_number(int low_size, int up_size, int k){
    int min = pow(2,low_size-1); // on genere les bornes inf et sup grace aux formules données
    int max = pow(2, up_size)-1;
    long rand = rand_long(min, max); // genere un nombre aleatoire entre min et max
    int i = is_prime_miller(rand, k);
    while (i!=1){
        rand=rand_long(min, max);
        i = is_prime_miller(rand, k); // test de Miller, si positif alors on retourne le nombre généré
    }
...
void print_long_vector(long *result, int size){
    printf ("Vector:[") ; // fonction fournie
    for (int i=0; i<size; i++){
        printf("%lx\t",result[i]) ;
    }
    printf ("]\n" ) ;
...
unsigned char *str_to_hash(const char * str){
    ...
    unsigned char *d = SHA256(str, strlen(str), 0); // cryptage de la chaine de char
    char *string = malloc(sizeof(char)*(SHA256_DIGEST_LENGTH*2+1)); 
    int cpt=0;
    for(int it = 0; it<SHA256_DIGEST_LENGTH; it++){ //transformation en en chaine de char pour facilter la manipulation 
        sprintf(string+cpt, "%02x", d[it]); // les elements sont ajoutés 2 à 2 car si c'est un chiffre %02 ajoute un 0 devant 
        cpt+=2;
    }
    return string;
}
